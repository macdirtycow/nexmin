#!/usr/bin/env bash
# Phase 8 ONAFHANKELIJK: QADBAK_PROVISIONER=native, no legacy hosting API API fallback.
#
# Usage:
...
#
# Revert: QADBAK_PROVISIONER=hybrid + QADBAK_LEGACY_API_FALLBACK=true + pm2 restart
set -euo pipefail
...
echo "==> Phase 8 INDEPENDENT (no legacy hosting API API fallback)"
...
echo "  provisioner=native, legacyApiFallback=false"
...
echo "  server admin embeds off (QADBAK_DISABLE_LEGACY_PANEL=true)"
echo "  apt remove legacy-panel: only after panel tests + snapshot"
