#!/usr/bin/env bash
# Cache zoneFile for every domain in native-domains.json (or from legacy hosting API).
set -euo pipefail
...
if [[ ${#domains[@]} -eq 0 ]] && command -v "${QADBAK_LEGACY_HOST_BIN:-}" &>/dev/null; then
  mapfile -t domains < <("${QADBAK_LEGACY_HOST_BIN}" list-domains --name-only 2>/dev/null | sed '/^$/d')
fi
