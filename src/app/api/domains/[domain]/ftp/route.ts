...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    const accounts = await getProvisioner().listFtpAccountsSafe(domain, session);
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    await getProvisioner().createFtpAccount(domain, body.user, body.pass, session);
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    await getProvisioner().updateFtpPassword(domain, body.user, body.pass, session);
...
export async function DELETE(request: Request, { params }: Params) {
  try {
    ...
    await getProvisioner().deleteFtpAccount(domain, body.user, session);
