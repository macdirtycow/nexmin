...
export async function GET() {
  try {
    ...
    return jsonOk({ admins: await getProvisioner().listAdmins(session) });
  } catch (err) {
...
export async function POST(request: Request) {
  try {
    ...
    await getProvisioner().createAdmin(body.user, body.pass, session);
...
export async function DELETE(request: Request) {
  try {
    ...
    await getProvisioner().deleteAdmin(body.user, session);
