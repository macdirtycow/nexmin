...
| 8d | `mail` | mail-* (legacy hosting API **CLI**, geen API) | Mailboxen |
...
| 8l | `php` | php-versions, php-directories, php-ini, php-set-directory | PHP-tab; **per-tenant PHP-FPM** (`apply-php-fpm-pool.sh`, nginx fastcgi) |
...
| Env | Gedrag |
|-----|--------|
| `QADBAK_MAIL_BACKEND=direct` | Geen `legacy-host` CLI (standaard bij `native` / fallback uit) |
| `QADBAK_MAIL_BACKEND=legacy-host` | Oude CLI (alleen hybrid + fallback) |

Debug: `sudo bash scripts/discover-mail-layout.sh example.com`

## Klaar voor `apt remove legacy-panel`

1. Alle features in `QADBAK_NATIVE_FEATURES` getest op testdomein.
2. `QADBAK_LEGACY_API_FALLBACK=false`
